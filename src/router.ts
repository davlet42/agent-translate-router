...
async function routeSegment(text: string, config: RouterConfig, options: TranslateOptions, prepared: PreparedPolicy, deadlineStart: number, discoveries: Map<ProviderId, Awaited<ReturnType<typeof discoverProvider>>>): Promise<{ text?: string; attempts: AttemptResult[]; provider?: ProviderId; model?: string; complete: boolean }> {
...
export async function translateText(text: string, config: RouterConfig, options: TranslateOptions = {}): Promise<TranslationResult> {
  ...
  for (const chunk of chunks) {
    ...
    const result = await routeSegment(chunk, config, options, prepared, start, discoveries);
...
export async function translateDocument(sourcePath: string, config: RouterConfig, options: TranslateOptions = {}): Promise<DocumentResult> {
  ...
  for (const chunk of chunks) {
    ...
    const result = await routeSegment(chunk, config, { ...options, event: "document" }, prepared, start, discoveries);
  ...
  if (config.cache.writeOwn) cachePath = await writeCache({ home: config.home, slug, sourcePath: absoluteSource, root, sourceSha256, body: finalText, sections: sectionMap });
